...
public class SquaredDividedFractalWorker extends AbstractFractalWorker {
    ...
    private final SquaredDividedFractalContext params;
    private final WorkPublisher publisher;

    public SquaredDividedFractalWorker(
            final int[] xpxs,
            final int[] ypxs,
            final SquaredDividedFractalContext params,
    ...
    final String calculate(
        ...
        for (; it < MAX_ITERATIONS_INT && abs <= MAX_VALUE; it++) {
            // z^2 + c/z
            final DoubleComplex val = DoubleComplex.divide(params.constant, z);
            z = DoubleComplex.multiply(z, z);
            z = DoubleComplex.add(z, val);
